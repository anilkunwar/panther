...
#include "SumEtasquare.h"

registerMooseObject("pantherApp", SumEtasquare);

template <>
InputParameters
validParams<SumEtasquare>()
{
  ...
  params.addRequiredParam<MaterialPropertyName>("h4_name","Switching function of sn");
...
SumEtasquare::SumEtasquare(const InputParameters & parameters)
...
SumEtasquare::computeValue()
{
  ...
  return _var1[_qp]*_var1[_qp]+_var2[_qp]*_var2[_qp]+_var3[_qp]*_var3[_qp]+_var4[_qp]*_var4[_qp] ;
}
